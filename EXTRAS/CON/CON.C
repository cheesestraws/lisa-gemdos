...
**
**  19 May 20   REM Horribly hacked this to work with Lisa GEM again.
** 	
** NAMES
** 
** 	JSL	Jason S. Loveman
...
int h;
{
	...
	return( insptr[h] > remptr[h] ? -1L : bconstat(h) );
}
...
//			glbkbchar[h][add[h]++ & KBBUFMASK] = ch;
        ...
        {                               /*                                  */
          if(kbchar[h] < KBBUFSZ)      /* Room?                            */
          {                             /************************************/
            *(insptr[h])++ = ch;         /* Yes, insert the character in buff*/
            kbchar[h]++;                 /* Up count                         */
          }                             /************************************/
...
int h;
{
    ...
    if(kbchar[h])
    {
        temp = *remptr[h]++;           /* Fetch the character    */
        kbchar[h]--;                   /* Decrement the count    */
        if(!kbchar[h])                 /* Gone to zero?          */
                remptr[h] = insptr[h] = &(glbkbchar[h][0]);
        return(temp);
    }
    return( bconin(h) );                 /* else get char from bios */
